...
默认会抽取 8 张代表帧，并写入转写稿的 `## Visual Frame Samples` 区块；同时会读取 `## Video Metadata`，并生成 `## Segment Evidence Map`，把语音分段与最近的代表帧绑定。整理稿生成时，应先查看这些图片和证据表，再描述：
...
```powershell
python scripts\organize.py --video "D:\path\to\video.mp4" --no-frame-ocr --language zh
```
...
```powershell
.\scripts\generate_report_from_notes.bat "D:\path\to\视频名_整理稿.md"
```
